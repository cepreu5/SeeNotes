...
try {
    ...
    const parsedItems = rawItems.map(raw => {
        // Regex to find "category": "..." (or unquoted key?)
        // The file has quoted keys "category": "..."
    ...
    // Target 'data' category as the special one (User referred to as data-management)
    const specialCat = 'data';

    uniqueCats.sort((a, b) => {
        if (a === specialCat) return 1;
        if (b === specialCat) return -1;
        // Treat 'data-management' as special too just in case
    ...
    console.log('Reordered blocks successfully. Categories found:', uniqueCats);
